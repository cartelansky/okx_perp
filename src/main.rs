...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(instruments) = data["data"].as_array() {
        for instrument in instruments {
            if let Some(instrument_id) = instrument["instId"].as_str() {
                if instrument_id.ends_with("-USDT-SWAP") {
                    let base_ccy = instrument_id.split('-').next().unwrap_or("");
                    coins.push(format!("OKX:{}USDT.P", base_ccy));
                }
            }
        }
    }
    ...
    let mut file = File::create("okx_usdt_perpetual_futures.txt")?;
    for coin in &coins {
        writeln!(file, "{}", coin)?;
    }

    println!("OKX USDT perpetual futures listesi başarıyla oluşturuldu ve kaydedildi.");
    println!(
